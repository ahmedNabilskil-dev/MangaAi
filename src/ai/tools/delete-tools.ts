...
// Template Delete Tools

export const deleteOutfitTemplateTool = ai.defineTool(
  {
    name: "deleteOutfitTemplate",
    description: "Deletes an outfit template permanently.",
    inputSchema: z.object({
      id: z.string().describe("ID of the outfit template to delete"),
    }),
    outputSchema: z.boolean().describe("True if deletion succeeded"),
  },
  async ({ id }) => {
    try {
      const exists = await getOutfitTemplate(id);
      if (!exists) {
        console.warn(`Outfit template ${id} not found`);
        return false;
      }

      await deleteOutfitTemplateService(id);
      return true;
    } catch (error) {
      console.error(`Error in deleteOutfitTemplateTool: ${error}`);
      return false;
    }
  }
);

export const deleteLocationTemplateTool = ai.defineTool(
  {
    name: "deleteLocationTemplate",
    description: "Deletes a location template permanently.",
    inputSchema: z.object({
      id: z.string().describe("ID of the location template to delete"),
    }),
    outputSchema: z.boolean().describe("True if deletion succeeded"),
  },
  async ({ id }) => {
    try {
      const exists = await getLocationTemplate(id);
      if (!exists) {
        console.warn(`Location template ${id} not found`);
        return false;
      }

      await deleteLocationTemplateService(id);
      return true;
    } catch (error) {
