 ...
 * It utilizes specific tools to interact with the backend service (now Dexie via db.ts).
 *
...
// Output now includes IDs as strings (UUIDs generated by Dexie service)
...
// --- Genkit Tools (Wrapping Dexie Service Functions) ---
...
// Tool to link existing characters to panels (using Dexie service function)
const assignCharacterToPanelTool = ai.defineTool({
    ...
    try {
        // Dexie service function updates the panel's characterIds array
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert manga storyteller and AI assistant using a Dexie database. Your task is to generate the structure for Chapter {{chapterNumber}}: "{{chapterTitle}}" for the project (ID: {{projectId}}) based *only* on the user's prompt.
