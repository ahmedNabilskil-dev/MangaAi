 ...
 * It utilizes specific tools to interact with the backend service (now abstracted via data-service.ts).
 *
...
// Import specific CRUD operations from the data service abstraction layer
import {
...
const CreateChapterFromPromptInputSchema = z.object({
  projectId: z.string().describe('The ID of the project to which the chapter will belong.'),
...
// Output now includes IDs as strings (UUIDs generated by data service)
...
// Helper to get projectId for context - now passed in input
async function getProjectIdForTool(context: { chapterId?: string, sceneId?: string, panelId?: string }): Promise<string | undefined> {
    ...
    console.warn("Could not determine projectId from context in getProjectIdForTool");
    return undefined; // Return undefined if it cannot be determined
}
...
export async function createChapterFromPrompt(input: CreateChapterFromPromptInput): Promise<CreateChapterFromPromptOutput> {
  // Input now requires projectId
  if (!input.projectId) {
      throw new Error("Project ID is required to create a chapter.");
  }
  return createChapterFromPromptFlow(input);
}

// --- Genkit Tools (Wrapping Data Service Functions) ---
...
const createPanelDialogueTool = ai.defineTool({
    ...
    inputSchema: z.object({
        ...
        style: z.object({
             bubbleType: z.enum(['normal', 'thought', 'scream', 'whisper', 'narration']).optional().describe("Visual style of the speech bubble."), // Added narration
...
const createCharacterTool = ai.defineTool({
    ...
    if (!input.mangaProjectId) {
        throw new Error("mangaProjectId is required to create a character.");
    }
...
// Tool to link existing characters to panels (using data service function)
const assignCharacterToPanelTool = ai.defineTool({
    ...
    try {
        // Data service function updates the panel's characterIds array
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert manga storyteller and AI assistant using a database (abstracted via tools). Your task is to generate the structure for Chapter {{chapterNumber}}: "{{chapterTitle}}" for the project (ID: {{projectId}}) based *only* on the user's prompt.
...
6.  **(Optional Character Creation):** If the prompt mentions a new character needed for a scene or panel, use the \`createCharacter\` tool FIRST, providing the \`mangaProjectId\` (which is {{projectId}}).
