...
interface EditorState {
  ...
  updatePage: (
    pageId: string,
    updates: Partial<Omit<Page, "id" | "shapes">>
  ) => void; // Update page settings like dimensions
...
const DEFAULT_PAGE_DIMENSIONS: PageDimensions = {
  width: 850,
  height: 1100,
  unit: "px",
  dpi: 96,
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  history: [
    {
      pages: JSON.parse(JSON.stringify([initialPage])),
    ...
    set(
      produce((state: EditorState) => {
        ...
        const newShapeWithDefaults: ShapeConfig = {
          ...
          width:
            shapeData.width ??
            (shapeData.type === "bubble" || shapeData.type === "text"
              ? 150
              : 200),
          height:
            shapeData.height ??
          ...
          fill:
            shapeData.fill ??
            (shapeData.type === "panel"
              ? "rgba(220, 220, 220, 0.5)"
              : shapeData.type === "bubble"
              ? "white"
          ...
          stroke:
            shapeData.stroke ??
            (shapeData.type === "text" || shapeData.type === "image"
              ? undefined
              : "black"),
          strokeWidth:
            shapeData.strokeWidth ?? (shapeData.type === "text" ? 0 : 1),
          ...
          props: {
            ...shapeData.props,
            ...((shapeData.type === "text" || shapeData.type === "bubble") &&
            !shapeData.props?.fontFamily
              ? { fontFamily: DEFAULT_FONT }
              : {}),
            ...((shapeData.type === "text" || shapeData.type === "bubble") &&
            !shapeData.props?.text
              ? { text: shapeData.type === "text" ? "New Text" : "Bubble" }
              : {}),
            ...((shapeData.type === "text" || shapeData.type === "bubble") &&
            !shapeData.props?.fontSize
          ...
          ...(shapeData.type === "image" && !shapeData.src
            ? {
                src: `https://picsum.photos/seed/${newId.substring(
                  0,
                  6
                )}/150/100`,
              }
  ...
  updateShape: (id, updates) =>
    set(
      produce((state: EditorState) => {
        ...
        const shapeIndex = state.pages[pageIndex].shapes.findIndex(
          (s) => s.id === id
        );
        if (shapeIndex !== -1) {
          ...
          if (
            "props" in newUpdates &&
            typeof newUpdates.props === "object" &&
            newUpdates.props !== null
          ) {
            const mergedProps = { ...currentShape.props, ...newUpdates.props };
            if (
              JSON.stringify(currentShape.props) !== JSON.stringify(mergedProps)
            ) {
          ...
          if (
            "fabricProps" in newUpdates &&
            typeof newUpdates.fabricProps === "object" &&
            newUpdates.fabricProps !== null
          ) {
            const mergedFabricProps = {
              ...currentShape.fabricProps,
              ...newUpdates.fabricProps,
            };
            if (
              JSON.stringify(currentShape.fabricProps) !==
              JSON.stringify(mergedFabricProps)
            ) {
              ...
              if (mergedFabricProps.selectable !== undefined)
                state.pages[pageIndex].shapes[shapeIndex].locked =
                  !mergedFabricProps.selectable;
              if (mergedFabricProps.visible !== undefined)
                state.pages[pageIndex].shapes[shapeIndex].visible =
                  mergedFabricProps.visible;
              changed = true;
            }
          ...
          for (const key in newUpdates) {
            if (Object.prototype.hasOwnProperty.call(newUpdates, key)) {
              ...
              if (currentShape[updateKey] !== newValue) {
                (state.pages[pageIndex].shapes[shapeIndex] as any)[updateKey] =
                  newValue;
        ...
        ids.forEach((id) => {
          ...
          if (shapeIndex !== -1) {
            ...
            if (
              ...
              if (
                JSON.stringify(currentShape.props) !==
            ...
            for (const key in newUpdates) {
              if (Object.prototype.hasOwnProperty.call(newUpdates, key)) {
                ...
                const newValue = (newUpdates as any)[updateKey];
                if (currentShape[updateKey] !== newValue) {
                  (state.pages[pageIndex].shapes[shapeIndex] as any)[
                    updateKey
                  ] = newValue;
                  if (updateKey === "locked") {
                    state.pages[pageIndex].shapes[shapeIndex].fabricProps = {
  ...
  deleteShapes: (ids) =>
    set(
      produce((state: EditorState) => {
        ...
        state.pages[pageIndex].shapes = state.pages[pageIndex].shapes.filter(
          (s) => !ids.includes(s.id)
        );
        ...
        if (state.pages[pageIndex].shapes.length < initialLength) {
          ...
          state.selectedShapeId =
            newSelectedIds.length === 1 ? newSelectedIds[0] : null;
          saveHistory(state);
        }
      })
    ),
    ...
    set(
      produce((state: EditorState) => {
        ...
        if (updates.dimensions) {
          ...
          if (
            JSON.stringify(currentPage.dimensions) !==
    ...
    set(
      produce((state: EditorState) => {
        if (state.pages.some((p) => p.id === pageId)) {
          ...
          console.warn(
            `Attempted to set current page to non-existent ID: ${pageId}`
          );
        }
      })
    ),
  ...
  bringToFront: (id) =>
    set(
      produce((state: EditorState) => {
        ...
        if (
          shapeIndex !== -1 &&
          shapeIndex < state.pages[pageIndex].shapes.length - 1
        ) {
  ...
  groupShapes: (ids) =>
    console.warn(
      "Group shapes action called, needs Fabric implementation",
      ids
    ),
  ungroupShape: (id) =>
    console.warn(
      "Ungroup shape action called, needs Fabric implementation",
      id
    ),
}));
