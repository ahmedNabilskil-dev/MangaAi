...
interface EnhancedProjectStructurePanelProps {
  ...
  onEntityDetailView?: (entity: DetailableEntity, entityType: string) => void; // New prop for detail view
}
...
export function EnhancedProjectStructurePanel({
  ...
  const showEntityDetails = (
    ...
    onEntityDetailView?.(entity, entityType as string);
  };

  const closeDetailPanel = () => {
    // This is no longer needed as detail panel is handled by parent
  };
                  ...
                  <div key={chapter._id} className="space-y-2">
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0 pr-16">
                        <button
                          onClick={() =>
                            toggleExpanded(`chapter-${chapter._id}`)
                          }
                          className="p-1 flex-shrink-0"
                        >
                          {expandedItems.has(`chapter-${chapter._id}`) ? (
                      ...
                      <div className="absolute right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900/90 backdrop-blur-sm rounded px-1 py-0.5">
                        ...
                        <button
                          onClick={() =>
                            handleEntitySelect(chapter, "chapter", chapter._id)
                          }
                          className={`p-1 rounded transition-colors ${
                            selectedEntity?.id === chapter._id &&
                            selectedEntity?.type === "chapter"
                    ...
                    {expandedItems.has(`chapter-${chapter._id}`) && (
                      ...
                      >
                        {chapter.scenes?.map((scene: Scene) => (
                          <div key={scene._id} className="space-y-2">
                            {/* Scene Header */}
                            <div
                              className={`flex items-center gap-3 p-3 rounded-lg transition-all hover:bg-gray-800/50 group relative bg-gray-800/20 border ${
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  toggleExpanded(`scene-${scene._id}`)
                                }
                                className="p-1 hover:bg-gray-700 rounded transition-colors flex-shrink-0"
                              >
                                {expandedItems.has(`scene-${scene._id}`) ? (
